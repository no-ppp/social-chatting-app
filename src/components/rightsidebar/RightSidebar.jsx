...
const RightSidebar = () => {
  ...
  const renderUser = (user) => (
    <div key={user.id} className={`relative flex items-center p-3 rounded-lg hover:bg-gray-700/50 cursor-pointer group transition-all duration-200 ${user.status === 'offline' ? 'opacity-50' : ''}`} onClick={(e) => handleUserClick(user, e)}>
      <div className="relative">
        <div className="w-10 h-10 bg-discord-dark rounded-full flex items-center justify-center text-lg font-medium shadow-lg">{user.name.charAt(0)}</div>
  ...
  return (
    <>
        ...
        <div className={`fixed right-0 top-0 w-72 h-screen bg-discord-sidebar transform transition-transform duration-300 ${state.isMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          <div className="p-4">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-gray-400 uppercase text-xs font-bold tracking-wider">Aktywni przyjaciele — {state.activeUsers.length}</h2>
      ...
      <div className="fixed right-0 top-0 w-0 h-screen bg-discord-sidebar border-l border-gray-800 transition-all duration-300 md:w-72">
        <div className="p-4">
          <h2 className="text-gray-400 uppercase text-xs font-bold mb-4 tracking-wider">Aktywni przyjaciele — {state.activeUsers.length}</h2>
          <div className="space-y-2">{state.activeUsers.map(renderUser)}</div>
          <h2 className="text-gray-400 uppercase text-xs font-bold mt-6 mb-2 tracking-wider">Offline — {state.offlineUsers.length}</h2>
          <div className="space-y-2">{state.offlineUsers.map(renderUser)}</div>
        </div>
